...
`snap`프로토콜은 [RLPx]위에서 동작하며, 피어들 사이에서 이더리움 상태 스냅샷을 교환하는 기능을 합니다.
본 프로토콜은 동적 스냅샷 포맷을 지원하는(또는 관심있는) 피어들을 에게 선택적으로 확장할 수 있습니다.
...
-->
`snap` 프로토콜은 준 실시간 데이터 조회를 위해 설계되었습니다. 프로토콜의 목적은 피어들에게 최근 상태에 대한 동적 스탭샷을 가능하게 하는 것 입니다.
`snap` 프로토콜은 
